...
import {
  ...
  Edit2, Trash2, LayoutDashboard, UserCheck, LogOut,
  ChevronLeft, ChevronRight, MoreHorizontal, Filter,
  AlertTriangle, CheckCircle2, BookMarked, Layers, Shield,
...
const JENJANG: Jenjang[] = [
  { id: 1, nama: "SD" }, { id: 2, nama: "SMP" }, { id: 3, nama: "SMA" },
];
...
const SISWA: Siswa[] = [
  { id: 1, id_jenjang: 3, nama: "Rani Kusuma", tanggal_lahir: "2007-04-12", jenis_kelamin: "P", no_hp: "082111111101", email: "rani@mail.com" },
  { id: 2, id_jenjang: 2, nama: "Farhan Adli", tanggal_lahir: "2009-08-22", jenis_kelamin: "L", no_hp: "082111111102", email: "farhan@mail.com" },
  { id: 3, id_jenjang: 3, nama: "Sinta Maharani", tanggal_lahir: "2006-11-03", jenis_kelamin: "P", no_hp: "082111111103", email: "sinta@mail.com" },
  { id: 4, id_jenjang: 1, nama: "Kevin Wijaya", tanggal_lahir: "2013-01-17", jenis_kelamin: "L", no_hp: "082111111104", email: "kevin@mail.com" },
  { id: 5, id_jenjang: 2, nama: "Nadia Putri", tanggal_lahir: "2010-06-30", jenis_kelamin: "P", no_hp: "082111111105", email: "nadia@mail.com" },
];
...
// ─── Shared UI ────────────────────────────────────────────────────────────────
const AVATAR_COLORS = [
  "bg-indigo-500", "bg-violet-500", "bg-sky-500", "bg-emerald-500", "bg-amber-500", "bg-rose-500",
];
function Avatar({ name, size = "md" }: { name: string; size?: "sm" | "md" | "lg" }) {
  const idx = name.charCodeAt(0) % AVATAR_COLORS.length;
  const sz = size === "sm" ? "w-7 h-7 text-xs" : size === "lg" ? "w-10 h-10 text-sm" : "w-8 h-8 text-xs";
  return (
    <div className={`${AVATAR_COLORS[idx]} ${sz} rounded-full flex items-center justify-center text-white font-semibold flex-shrink-0`}>
      {initials(name)}
    </div>
  );
}
...
function Badge({ label, variant = "default" }: { label: string; variant?: "default" | "success" | "warning" | "danger" | "info" }) {
  ...
  return <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${styles[variant]}`}>{label}</span>;
}

function Button({
  children, onClick, variant = "primary", size = "md", disabled = false, icon, className = "", type = "button",
}: {
  children: React.ReactNode; onClick?: () => void; variant?: "primary" | "secondary" | "ghost" | "danger";
  size?: "sm" | "md"; disabled?: boolean; icon?: React.ReactNode; className?: string; type?: "button" | "submit";
}) {
  ...
  const variants = {
    primary: "bg-[#4361EE] text-white hover:bg-[#3451DB] focus:ring-[#4361EE]/50 shadow-sm",
...
function Input({ label, value, onChange, type = "text", placeholder, required, min, max }: {
  label?: string; value: string; onChange: (v: string) => void;
  type?: string; placeholder?: string; required?: boolean; min?: string; max?: string;
}) {
  return (
    <div className="flex flex-col gap-1">
      {label && <label className="text-xs font-medium text-slate-600">{label}{required && <span className="text-red-400 ml-0.5">*</span>}</label>}
      <input
...
function Toast({ message, type, onClose }: { message: string; type: "success" | "error"; onClose: () => void }) {
  ...
  return (
    <div className={`fixed top-5 right-5 z-50 flex items-start gap-3 px-4 py-3 border rounded-xl shadow-xl max-w-sm ${styles}`}>
      <Icon size={18} className={`flex-shrink-0 mt-0.5 ${iconColor}`} />
      <p className="text-sm font-medium leading-snug">{message}</p>
      <button onClick={onClose} className="ml-auto flex-shrink-0 hover:opacity-70"><X size={14} /></button>
    </div>
  );
}

// ─── Sidebar ──────────────────────────────────────────────────────────────────
const NAV: Record<Role, { icon: React.ElementType; label: string; page: string }[]> = {
  student: [
...
function Sidebar({ role, page, setPage }: { role: Role; page: string; setPage: (p: string) => void }) {
  const items = NAV[role];
  const roleLabel = { student: "Student Portal", teacher: "Teacher Portal", admin: "Admin Console" }[role];
  const roleColor = { student: "bg-sky-500", teacher: "bg-violet-500", admin: "bg-indigo-600" }[role];
  return (
    <aside className="w-56 flex-shrink-0 h-screen flex flex-col bg-[#1C2B3A] fixed left-0 top-0 z-20">
      <div className="px-5 py-5 border-b border-white/10">
        <div className="flex items-center gap-2.5">
          <div className={`w-8 h-8 rounded-lg ${roleColor} flex items-center justify-center flex-shrink-0`}>
...
// ─── Top Bar ──────────────────────────────────────────────────────────────────
function TopBar({ role, setRole, globalSearch, setGlobalSearch, onLogout, loggedInName }: {
  role: Role; setRole: (r: Role) => void; globalSearch: string; setGlobalSearch: (s: string) => void;
  onLogout: () => void; loggedInName: string;
}) {
  const roleUser = {
      ...
      <div className="flex items-center gap-3 ml-auto">
        ...
        <div className="flex items-center gap-2 pl-2 border-l border-slate-200">
          ...
          <div className="hidden xl:block">
            <p className="text-xs font-semibold text-slate-700 leading-tight">{roleUser.name}</p>
            <p className="text-xs text-slate-400 leading-tight">{roleUser.badge}</p>
          </div>
          <button onClick={onLogout} title="Sign out" className="ml-1 p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-all">
            <LogOut size={14} />
          </button>
        </div>
      </div>
        ...
        <div className="flex items-center gap-0">
          {["Education Level & Subject", "Choose Teacher", "Availability", "Schedule Details"].map((s, i) => (
            <div key={s} className="flex items-center flex-1 last:flex-none">
              ...
              {i < 3 && <div className={`flex-1 h-px mx-3 ${step > i + 1 ? "bg-emerald-400" : "bg-slate-200"}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 space-y-4">
          ...
          {selJenjang && selMapel && (
            <div className="flex justify-end">
              <Button onClick={() => setStep(2)} size="sm">Next: Choose Teacher <ChevronRight size={14} /></Button>
            </div>
          )}
        </div>
            ...
            <div className="p-5 space-y-4">
              ...
              {summaryComplete && (
                <div className={`rounded-lg p-3 text-xs font-medium flex items-center gap-2 ${booked ? "bg-emerald-50 text-emerald-700" : "bg-indigo-50 text-[#4361EE]"}`}>
                  {booked ? <CheckCircle2 size={14} /> : <AlertTriangle size={14} />}
                  {booked ? "Your lessons have been confirmed!" : "Ready to book. Review and confirm."}
                </div>
              )}
            </div>
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                {["Subject", "Teacher", "Date", "Time", "Duration", "Status"].map((h) => (
...
function TeacherAvailability() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-xl font-bold text-slate-800">Manage Availability</h1>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Weekly overview of your confirmed lesson bookings.</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="secondary" size="sm" icon={<ChevronLeft size={14} />}>Prev</Button>
          <span className="text-sm font-medium text-slate-700 px-2">Week of Jan 20, 2025</span>
        ...
        <div className="grid grid-cols-7 divide-x divide-slate-100 border-b border-slate-200">
          {weekDates.map(({ hari, date }) => (
            <div key={hari} className="px-3 py-3 text-center">
              <p className="text-xs text-slate-400 font-medium">{hari.slice(0, 3).toUpperCase()}</p>
              <p className="text-base font-bold text-slate-700 mt-0.5">{date.getDate()}</p>
            </div>
          ))}
        </div>
              ...
              <div key={hari} className="p-2 space-y-1.5 min-h-[180px]">
                ...
                {dayLessons.map((les, i) => {
                  ...
                  return (
                    ...
                    >
                      <p className="text-xs font-bold">{start} – {end}</p>
                      <p className="text-xs opacity-75 truncate mt-0.5">{siswa?.nama}</p>
                    </button>
                  );
                })}
              </div>
      ...
      {selLesson && (() => {
        ...
        return (
          <div className="fixed inset-0 z-40 bg-black/30 flex items-center justify-center p-6" onClick={() => setSelLesson(null)}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs text-slate-400 uppercase tracking-wider font-medium">Lesson Detail</p>
                  <h3 className="text-lg font-bold text-slate-800 mt-0.5">{mapel?.nama} <span className="text-slate-400 font-normal text-base">({jenjang?.nama})</span></h3>
                </div>
                <button onClick={() => setSelLesson(null)} className="p-1.5 hover:bg-slate-100 rounded-lg"><X size={16} /></button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-indigo-50 rounded-xl p-3">
                  ...
                  <p className="text-sm font-semibold text-slate-700 mt-0.5">{formatDate(selLesson.tanggal_mulai)}</p>
                </div>
                <div className="bg-indigo-50 rounded-xl p-3">
                  <p className="text-xs text-slate-500">Time</p>
                  <p className="text-sm font-semibold text-slate-700 mt-0.5">{start} – {end}</p>
                </div>
              </div>
            </div>
          </div>
...
function AdminStudents({ search }: { search: string }) {
  ...
  const filtered = students
    .filter((s) => [s.nama, s.email, s.no_hp].join(" ").toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => {
    ...
    <div className="space-y-5 relative">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Manage all registered students in the system.</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
          <p className="text-sm text-slate-500">{filtered.length} of {students.length} students</p>
          <Button variant="ghost" size="sm" icon={<SlidersHorizontal size={14} />}>Filter</Button>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100">
          ...
          <tbody>
            {filtered.map((s) => (
              <tr key={s.id} className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors group">
                ...
                <td className="px-5 py-3.5"><Badge label={jenjangMap[s.id_jenjang] ?? "—"} variant="info" /></td>
                <td className="px-5 py-3.5 text-slate-600">{s.jenis_kelamin === "L" ? "Male" : "Female"}</td>
                <td className="px-5 py-3.5 text-slate-500">{s.tanggal_lahir}</td>
                <td className="px-5 py-3.5 text-slate-600">{s.email}</td>
                <td className="px-5 py-3.5 text-slate-600">{s.no_hp}</td>
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => openEdit(s)} className="p-1.5 text-slate-400 hover:text-[#4361EE] hover:bg-indigo-50 rounded-lg transition-all"><Edit2 size={13} /></button>
                    <button onClick={() => handleDelete(s.id)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all"><Trash2 size={13} /></button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-96 bg-white shadow-2xl z-40 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              <Input label="Full Name" value={form.nama ?? ""} onChange={(v) => setForm((p) => ({ ...p, nama: v }))} required />
...
function AdminTeachers({ search }: { search: string }) {
  ...
  const filtered = teachers.filter((t) =>
    [t.nama, t.email, t.no_hp].join(" ").toLowerCase().includes(search.toLowerCase())
  );

  function openAdd() { setForm({}); setFormSubject(""); setFormEduLevel(""); setEditItem(null); setDrawer("add"); }
  function openEdit(g: Guru) { setForm(g); setFormSubject(""); setFormEduLevel(""); setEditItem(g); setDrawer("edit"); }
  function handleDelete(id: number) { setTeachers((p) => p.filter((g) => g.id !== id)); }
  function handleSave() {
    if (drawer === "add") setTeachers((p) => [...p, { ...form, id: Date.now() } as Guru]);
    else if (editItem) setTeachers((p) => p.map((g) => g.id === editItem.id ? { ...g, ...form } as Guru : g));
    setDrawer(null);
  }
    ...
    <div className="space-y-5 relative">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Manage all registered tutors and their expertise.</p>
        </div>
        <Button onClick={openAdd} icon={<Plus size={15} />}>Add Teacher</Button>
      </div>
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((g) => {
              const expertise = KEAHLIAN.filter((k) => k.id_guru === g.id).slice(0, 2).map((k) => {
                const mp = MAPEL.find((m) => m.id === k.id_mapel);
                const jj = JENJANG.find((j) => j.id === k.id_jenjang);
                ...
                <tr key={g.id} className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors group">
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex gap-1 flex-wrap">
                      {expertise.map((e) => <Badge key={e} label={e} variant="info" />)}
                    </div>
                  </td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openEdit(g)} className="p-1.5 text-slate-400 hover:text-[#4361EE] hover:bg-indigo-50 rounded-lg transition-all"><Edit2 size={13} /></button>
                      <button onClick={() => handleDelete(g.id)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all"><Trash2 size={13} /></button>
                    </div>
                  </td>
                </tr>
              );
      ...
      {drawer && (
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-96 bg-white shadow-2xl z-40 flex flex-col">
            <div className="px-6 py-5 border-b border-slate-200 flex items-center justify-between">
              <h2 className="text-base font-bold text-slate-800">{drawer === "add" ? "Add New Teacher" : "Edit Teacher"}</h2>
              <button onClick={() => setDrawer(null)} className="p-1.5 hover:bg-slate-100 rounded-lg"><X size={16} /></button>
            </div>
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              <Input label="Full Name" value={form.nama ?? ""} onChange={(v) => setForm((p) => ({ ...p, nama: v }))} required />
              ...
              <div className="pt-2 border-t border-slate-100">
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Expertise</p>
                <div className="space-y-3">
                  <Select label="Subject" value={formSubject} onChange={setFormSubject}
                    options={Array.from(new Map(MAPEL.map((m) => [m.nama, m])).values()).map((m) => ({ value: m.nama, label: m.nama }))}
                    placeholder="Select subject" />
                  <Select label="Education Level" value={formEduLevel} onChange={setFormEduLevel}
                    options={JENJANG.map((j) => ({ value: j.nama, label: j.nama }))}
            ...
            <div className="px-6 py-4 border-t border-slate-200 flex gap-3">
              <Button variant="secondary" onClick={() => setDrawer(null)} className="flex-1 justify-center">Cancel</Button>
              <Button onClick={handleSave} className="flex-1 justify-center">{drawer === "add" ? "Add Teacher" : "Save Changes"}</Button>
            </div>
          </div>
        </>
      )}
...
function AdminAdmins({ search }: { search: string }) {
  ...
  const filtered = admins.filter((a) =>
    [a.nama, a.email, a.no_hp].join(" ").toLowerCase().includes(search.toLowerCase())
  );
  function handleSave() {
  ...
  return (
    <div className="space-y-5 relative">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { setForm({}); setDrawer("add"); }} icon={<Plus size={15} />}>Add Admin</Button>
      </div>
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead>
          ...
          <tbody>
            {filtered.map((a) => (
              <tr key={a.id} className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors">
                ...
                <td className="px-5 py-3.5"><Badge label="Administrator" variant="warning" /></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {drawer && (
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-96 bg-white shadow-2xl z-40 flex flex-col">
            ...
            <div className="px-6 py-4 border-t border-slate-200 flex gap-3">
              <Button variant="secondary" onClick={() => setDrawer(null)} className="flex-1 justify-center">Cancel</Button>
              <Button onClick={handleSave} className="flex-1 justify-center">Add Admin</Button>
            </div>
          </div>
        </>
      )}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-5">
        {[
          ...
          { label: "Upcoming", value: String(myLes.filter((l) => new Date(l.tanggal_mulai) > new Date()).length), sub: "Scheduled", color: "bg-sky-50", iconColor: "text-sky-500", Icon: Calendar },
          { label: "Completed", value: String(myLes.filter((l) => new Date(l.tanggal_mulai) <= new Date()).length), sub: "Finished", color: "bg-emerald-50", iconColor: "text-emerald-500", Icon: CheckCircle2 },
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-3">
          ...
          <div className="space-y-2">
            <button onClick={() => setPage("book")} className="w-full flex items-center justify-between px-4 py-3 bg-indigo-50 hover:bg-indigo-100 rounded-xl transition-colors text-[#4361EE] font-medium text-sm">
              <span className="flex items-center gap-2"><BookOpen size={16} />Book a New Lesson</span>
            ...
            <button onClick={() => setPage("mylessons")} className="w-full flex items-center justify-between px-4 py-3 bg-slate-50 hover:bg-slate-100 rounded-xl transition-colors text-slate-700 font-medium text-sm">
              <span className="flex items-center gap-2"><Calendar size={16} />View All My Lessons</span>
              <ChevronRight size={16} />
            </button>
          </div>
        </div>
...
function TeacherDashboard() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-slate-800">Welcome, {guru.nama}!</h1>
        <p className="text-sm text-slate-500 mt-0.5">Overview of your teaching schedule and students.</p>
      </div>
...
function AdminDashboard() {
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-5">
        {[
          { label: "Total Students", value: String(SISWA.length), Icon: GraduationCap, color: "bg-sky-50 text-sky-500" },
          { label: "Total Teachers", value: String(GURU.length), Icon: UserCheck, color: "bg-violet-50 text-violet-500" },
          { label: "Total Lessons", value: String(LES_DATA.length), Icon: BookOpen, color: "bg-indigo-50 text-[#4361EE]" },
          { label: "Admins", value: String(ADMINS.length), Icon: Shield, color: "bg-emerald-50 text-emerald-500" },
        ].map(({ label, value, Icon, color }) => (
          <div key={label} className="bg-white border border-slate-200 rounded-xl p-5 flex items-center gap-4">
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${color.split(" ")[0]}`}>
              <Icon size={20} className={color.split(" ")[1]} />
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100">
          <p className="text-sm font-semibold text-slate-700">Recent Lesson Bookings</p>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100">
            ...
            {LES_DATA.map((les) => {
              ...
              return (
                <tr key={les.id} className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-slate-500">{les.tanggal_mulai.split("T")[0]}</td>
                  <td className="px-5 py-3.5 text-slate-500">{toHHMM(les.tanggal_mulai)} – {toHHMM(les.tanggal_selesai)}</td>
                </tr>
              );
            })}
          </tbody>
        ...
        <div className="space-y-6">
          <div className="space-y-3">
            ...
            <p className="text-white/50 text-sm leading-relaxed">
              Book lessons, track schedules, and manage students and teachers — all in one place.
            </p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {[
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              ...
              <button onClick={onGoRegister} className="text-[#4361EE] font-medium hover:underline underline-offset-2">
                Register here
              </button>
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-slate-600">Password <span className="text-red-400">*</span></label>
              <div className="relative">
                <input
                ...
                />
                <button type="button" onClick={() => setShowPwd((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 text-xs font-medium">
                  {showPwd ? "Hide" : "Show"}
                </button>
              </div>
            </div>
            ...
            {[
              ...
              <div key={s} className="flex items-start gap-3">
                <div className="w-6 h-6 bg-[#4361EE]/20 border border-[#4361EE]/30 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                  <span className="text-xs font-bold text-[#4361EE]">{s}</span>
                </div>
                <div>
        ...
        <div className="w-full max-w-md space-y-7">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              ...
              <button onClick={onGoLogin} className="text-[#4361EE] font-medium hover:underline underline-offset-2">Sign in</button>
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            {["Account Type & Credentials", "Profile Details"].map((s, i) => (
              <div key={s} className="flex items-center gap-2 flex-1">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${step > i + 1 ? "bg-emerald-500 text-white" : step === i + 1 ? "bg-[#4361EE] text-white" : "bg-slate-200 text-slate-400"}`}>
          ...
          {step === 1 && (
            <form onSubmit={handleStep1} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(["student", "teacher", "admin"] as Role[]).map((r) => {
                    ...
                    return (
                      <button
                        key={r} type="button" onClick={() => setRegRole(r)}
                        className={`flex flex-col items-center gap-1.5 py-3 px-2 rounded-xl border-2 transition-all ${role === r ? roleColors[r] + " border-current" : "border-slate-200 text-slate-500 hover:border-slate-300 hover:bg-slate-50"}`}
                      >
                        <Icon size={18} />
              ...
              <button type="submit" className="w-full py-2.5 bg-[#4361EE] hover:bg-[#3451DB] text-white text-sm font-semibold rounded-xl transition-all shadow-sm">
                Continue
              </button>
            </form>
          )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {role === "student" && (
                <>
                  ...
                  <Select label="Gender" value={form.jenis_kelamin} onChange={f("jenis_kelamin")}
                    options={[{ value: "L", label: "Male" }, { value: "P", label: "Female" }]}
                    placeholder="Select gender" required />
                  <Select label="Education Level" value={form.id_jenjang} onChange={f("id_jenjang")}
              ...
              {role === "teacher" && (
                <div className="pt-1 border-t border-slate-100">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Expertise</p>
                  <div className="space-y-3">
                    <Select label="Subject" value={form.id_jenjang ?? ""} onChange={f("id_jenjang")}
                      options={Array.from(new Map(MAPEL.map((m) => [m.nama, m])).values()).map((m) => ({ value: m.nama, label: m.nama }))}
                      placeholder="Select subject" required />
                    <Select label="Education Level" value={form.jenis_kelamin ?? ""} onChange={f("jenis_kelamin")}
                      options={JENJANG.map((j) => ({ value: j.nama, label: j.nama }))}
                      placeholder="Select level" required />
                  </div>
                </div>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" className="flex-1 py-2 bg-[#4361EE] hover:bg-[#3451DB] text-white text-sm font-semibold rounded-xl transition-all shadow-sm">
                  Create Account
                </button>
              </div>
            </form>
...
export default function App() {
  ...
  if (authScreen === "login") return <LoginPage onLogin={handleLogin} onGoRegister={() => setAuthScreen("register")} />;
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FA]" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="pl-56 flex flex-col min-h-screen">
        <TopBar
          role={loggedInRole} setRole={handleRoleSwitch}
          globalSearch={globalSearch} setGlobalSearch={setGlobalSearch}
          onLogout={handleLogout} loggedInName={loggedInName}
        />
